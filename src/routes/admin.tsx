...
export const Route = createFileRoute("/admin")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "إدارة متجر هاشم للطيب، المنتجات، الطلبات، الفروع، الإعدادات واللوجو." },
    ],
  }),
...
function AdminLayout() {
    ...
    return (
      <div className="mx-auto max-w-md px-6 py-24 text-center">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          {pick("هذه الصفحة مخصصة لمدير متجر هاشم للطيب فقط.", "This area is for administrators only.")}
        </p>
        <Link
          to="/auth"
  ...
  const tab = "rounded-xl px-4 py-2 text-xs font-semibold text-muted-foreground transition-all hover:bg-accent hover:text-foreground";
